...
app.post('/api/fetch', async (req, res) => {
  try {
    ...
    // Look for multiple content blocks and combine them
    ...
    for (const selector of contentBlockSelectors) {
      ...
      if (elements.length > 0) {
        ...
        elements.forEach(el => {
          ...
          // Skip empty or very short blocks (likely navigation/UI elements)
    ...
    // Strategy 2: Use Readability as fallback or alternative
    try {
      const documentClone = doc.cloneNode(true);
      const reader = new Readability(documentClone, {
        charThreshold: 25,     // Very low threshold
        keepClasses: true,
        nbTopCandidates: 10    // Consider more candidate elements
      });
      ...
      if (article) {
        ...
        // Use Readability's content if it's longer or if we found nothing else
    ...
    // Fallback: If we still have nothing, try to get the body
    if (!htmlContent) {
